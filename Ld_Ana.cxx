...
//    }
    ...
    if(centnumber >= 4 && centnumber <= 8)
    {
        ...
        {
            if(isinpTBin(pt, 1) && isinRapBin(rap, 2+ii)) InvMassv2_pt_cent[8-(int)centnumber][ii]->Fill(pt, cos(2*mPhiPsi), invM, gweight);
        }
    }
...
void Ld_Ana::DefHistograms()
{
    ...
    for(int icent = 0; icent < 5; icent++){
        for(int i = 0; i < 10; i++)
        {
            InvMassv2_pt_cent[icent][i] = new TH3F(Form("InvMassv2_yptmode%d_pt_diffy_cent%d", i, icent),"", 100,0.0,5.0,200,-1.0,1.0, 100, 1.090, 1.150);
        }
    }
